...
{
    public class ServiceTypeOwnerGroupLocationsController : BaseODataController
    {
        private readonly ServiceTypeOwnerGroupLocationBLL _serviceTypeOwnerGroupLocationBLL;
        public ServiceTypeOwnerGroupLocationsController(ServiceTypeOwnerGroupLocationBLL serviceTypeOwnerGroupLocationBLL)
        {
            if (serviceTypeOwnerGroupLocationBLL != null)
            {
                _serviceTypeOwnerGroupLocationBLL = serviceTypeOwnerGroupLocationBLL;
            }
            else
            {
                throw new ArgumentNullException("ServiceTypeOwnerGroupLocationBLL is null.");
            }
        }

        // GET: odata/ServiceTypeOwnerGroupLocations
        [EnableQuery(MaxExpansionDepth = 5)]
        public IQueryable<ServiceTypeOwnerGroupLocation> GetServiceTypeOwnerGroupLocations()
        {
            return _serviceTypeOwnerGroupLocationBLL.GetServiceTypeOwnerGroupLocations();
        }

        // GET: odata/ServiceTypeOwnerGroupLocations(5)
        [EnableQuery]
        public SingleResult<ServiceTypeOwnerGroupLocation> GetServiceTypeOwnerGroupLocation([FromODataUri] int? key)
        {
            return SingleResult.Create(_serviceTypeOwnerGroupLocationBLL.Find(key));
        }
