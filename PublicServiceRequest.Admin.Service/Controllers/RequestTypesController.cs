...
{
    public class RequestTypesController : BaseODataController
    {
        private readonly RequestTypeBLL _requestTypeBLL;
        public RequestTypesController(RequestTypeBLL requestTypeBLL)
        {
            if (requestTypeBLL != null)
            {
                _requestTypeBLL = requestTypeBLL;
            }
            else
            {
                throw new ArgumentNullException("RequestTypeBLL is null.");
            }
        }

        // GET: odata/RequestTypes
        [EnableQuery(MaxExpansionDepth = 5)]
        public IQueryable<RequestType> GetRequestTypes()
        {
            return _requestTypeBLL.GetRequestTypes();
        }

        // GET: odata/RequestTypes(5)
        [EnableQuery]
        public SingleResult<RequestType> GetRequestType([FromODataUri] int? key)
        {
            return SingleResult.Create(_requestTypeBLL.Find(key));
        }
