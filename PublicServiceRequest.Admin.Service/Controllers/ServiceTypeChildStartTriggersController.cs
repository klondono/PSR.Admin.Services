...
{
    public class ServiceTypeChildStartTriggersController : BaseODataController
    {
        private readonly ServiceTypeChildStartTriggerBLL _serviceTypeChildStartTriggerBLL;
        public ServiceTypeChildStartTriggersController(ServiceTypeChildStartTriggerBLL serviceTypeChildStartTriggerBLL)
        {
            if (serviceTypeChildStartTriggerBLL != null)
            {
                _serviceTypeChildStartTriggerBLL = serviceTypeChildStartTriggerBLL;
            }
            else
            {
                throw new ArgumentNullException("ServiceTypeChildStartTriggerBLL is null.");
            }
        }

        // GET: odata/ServiceTypeChildStartTriggers
        [EnableQuery(MaxExpansionDepth = 5)]
        public IQueryable<ServiceTypeChildStartTrigger> GetServiceTypeChildStartTriggers()
        {
            return _serviceTypeChildStartTriggerBLL.GetServiceTypeChildStartTriggers();
        }

        // GET: odata/ServiceTypeChildStartTriggers(5)
        [EnableQuery]
        public SingleResult<ServiceTypeChildStartTrigger> GetServiceTypeChildStartTrigger([FromODataUri] int? key)
        {
            return SingleResult.Create(_serviceTypeChildStartTriggerBLL.Find(key));
        }
