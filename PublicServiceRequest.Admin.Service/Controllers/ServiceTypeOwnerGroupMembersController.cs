...
{
    public class ServiceTypeOwnerGroupMembersController : BaseODataController
    {
        private readonly ServiceTypeOwnerGroupMemberBLL _serviceTypeOwnerGroupMemberBLL;
        public ServiceTypeOwnerGroupMembersController(ServiceTypeOwnerGroupMemberBLL serviceTypeOwnerGroupMemberBLL)
        {
            if (serviceTypeOwnerGroupMemberBLL != null)
            {
                _serviceTypeOwnerGroupMemberBLL = serviceTypeOwnerGroupMemberBLL;
            }
            else
            {
                throw new ArgumentNullException("ServiceTypeOwnerGroupMemberBLL is null.");
            }
        }

        // GET: odata/ServiceTypeOwnerGroupMembers
        [EnableQuery(MaxExpansionDepth = 5)]
        public IQueryable<ServiceTypeOwnerGroupMember> GetServiceTypeOwnerGroupMembers()
        {
            return _serviceTypeOwnerGroupMemberBLL.GetServiceTypeOwnerGroupMembers();
        }

        // GET: odata/ServiceTypeOwnerGroupMembers(5)
        [EnableQuery]
        public SingleResult<ServiceTypeOwnerGroupMember> GetServiceTypeOwnerGroupMember([FromODataUri] int? key)
        {
            return SingleResult.Create(_serviceTypeOwnerGroupMemberBLL.Find(key));
        }
