...
{
    public class RequestStatusesController : BaseODataController
    {
        private readonly RequestStatusBLL _requestStatusBLL;
        public RequestStatusesController(RequestStatusBLL requestStatusBLL)
        {
            if (requestStatusBLL != null)
            {
                _requestStatusBLL = requestStatusBLL;
            }
            else
            {
                throw new ArgumentNullException("RequestStatusBLL is null.");
            }
        }

        // GET: odata/RequestStatuses
        [EnableQuery(MaxExpansionDepth = 5)]
        public IQueryable<RequestStatus> GetRequestStatuses()
        {
            return _requestStatusBLL.GetRequestStatuses();
        }

        // GET: odata/RequestStatuses(5)
        [EnableQuery]
        public SingleResult<RequestStatus> GetRequestStatus([FromODataUri] int? key)
        {
            return SingleResult.Create(_requestStatusBLL.Find(key));
        }
